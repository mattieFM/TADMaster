...
from .forms import DataForm, NormForm, CallerForm, UploadFileForm, ExistingForm, ExistingFormNoMatrix, UploadBedForm, UploadZipForm
...
protected_visualizations = [389, 392, 393, 421, 395, 396, 397, 413, 414, 415, 379, 374, 371, 370, 367, 366, 365, 359, 412, 358, 352, 357, 387, 479, 505, 522, 608]
...
def fail_data(request, pk):
    """set the status of a job to failure"""
    if request.method == 'POST':
        data = Data.objects.get(pk=pk)
...
def comfirmation(request, pk):
    ...
    if request.is_ajax():
        job_path = os.path.join('/var/www/html/TADMaster/Site/storage/data/', "job_%s" % str(data.job_id))
        ...
        if form.is_valid():
            ...
            return JsonResponse({
                'success': 'True',
                'url': "/TADMaster"+reverse('processing', args=[data.pk]),
            })
        else:
            return JsonResponse({
                'success': 'False',
...
def input(request):
    ...
    context["error"] = "none"
    if request.method == 'POST':
        logger.debug("tadMaster input")
        form = ExistingFormNoMatrix(request.POST, request.FILES)
        if form.is_valid():
            logger.debug("Start FORM")
            logger.debug(form)
            logger.debug("End FORM")
            data = form.save()
            return redirect('upload_bed', pk=data.pk)
    else:
        form = ExistingFormNoMatrix()
        print("FORM FAIL")
    context["form"] = form	
    return render(request, 'manyTAD/input.html', context)
    ...
    if request.is_ajax():
        ...
        if form.is_valid() and request.POST.get('data_input_type') is not None:
            ...
            if context["error"] == "none":
                ...
                return JsonResponse({
                    'success': 'True',
                    'url': "/TADMaster"+reverse('upload_bed', args=[data.pk]),
                })
        elif request.POST.get('data_input_type') is None:
            context["error"] = "input type error"
            return JsonResponse({
...
def check_uploaded_bed(f, title, path):
    try:
        with open(path +'/'+ title + '.bed', 'r') as file:
            ...
            if len(tad_data[0]) == 2:
                tad_data = np.asarray(tad_data, dtype='float')
                        ...
                        for i in range(len(tad_data)):
                                if not re.match("(^chr(1[0-9]|[1-9]|[2][0-3])$)", tad_data[i][0]):
                                    return False
                                temp_data.append([tad_data[i][1].strip(), tad_data[i][2].strip()])
                        tad_data = np.asarray(temp_data, dtype='float')
        return tad_data.size != 0
    except:
        return False	

def delete_uploaded_bed(f, title, path):
    if os.path.exists(path +'/'+ title + '.bed'):
        os.remove(path +'/'+ title + '.bed')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if str(data.document):
                ...
                final_out_path = os.path.join(output_path, "NoMatrixProvided")
            if not os.path.isdir(final_out_path):
                os.mkdir(final_out_path)
            handle_uploaded_bed(request.FILES['bed_file_1'], request.POST.get('bed_title_1'), final_out_path)
            if not check_uploaded_bed(request.FILES['bed_file_1'], request.POST.get('bed_title_1'), final_out_path):
                context["error"] = "Format error file 1"
                data.status = "Failure"
                data.save()
                delete_uploaded_bed(request.FILES['bed_file_1'], request.POST.get('bed_title_1'), final_out_path)
            handle_uploaded_bed(request.FILES['bed_file_2'], request.POST.get('bed_title_2'), final_out_path)
            if not check_uploaded_bed(request.FILES['bed_file_2'], request.POST.get('bed_title_2'), final_out_path):
                context["error"] = context["error"] + " " +"Format error file 2"
                data.status = "Failure"
                data.save()
                delete_uploaded_bed(request.FILES['bed_file_2'], request.POST.get('bed_title_2'), final_out_path)
            if context["error"] == "":
        ...
        form = UploadBedForm()
    context["form"] = form
    return render(request, 'manyTAD/upload_bed.html', context)
...
def upload_previous_job(request):
    ...
    if request.is_ajax():
        ...
        if form.is_valid():
            ...
            data.save()
            path = '/var/www/html/TADMaster/Site/storage/data/'
            ...
            os.chdir('/var/www/html/TADMaster/Site/storage/data/')
                ...
                with open('/var/www/html/TADMaster/Site/manyTAD/jobs.log', 'r') as fh:
                    ...
                    for line in lines:
                        if str(hash) == line.strip():
                            valid_hash = True
                if valid_hash:
                    new_dir_path = "/var/www/html/TADMaster/Site/storage/data/" + 'job_' + str(data.job_id)
                    ...
                    return JsonResponse({
                        'success': 'True',
                        'url': "/TADMaster"+reverse('visualize', args=[data.pk]),
                    })
...
def download(request, pk):
    ...
    shutil.make_archive(path, 'zip', '/var/www/html/TADMaster/Site/storage/data/', "job_%s" % str(data.job_id))
    shutil.move("/var/www/html/TADMaster/Site/storage/data/job_%s" % str(data.job_id)+".zip", path)
...
# Calculate true position in queue	
        ...
        if(data.status == "job_failure"):
            context['position'] = "Your job has timed out or failed"
            context['fail'] = True
            context['id'] = pk
