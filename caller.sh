...
then
	...
	rm -rf $armatus_input
	cd $home_path
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/Armatus.bed >> $log_path
...
then
	...
	taskset -c 0-100 java -jar juicer_tools_1.22.01.jar arrowhead -c $chr  -r $resolution --threads 0 -k arrowhead_norm $hic_path $additional_file_path/Arrowhead.bed --ignore-sparsity
	cd $home_path
...
then
	...
	#NxN to NxN+3 format
	taskset -c 0-100 python3 preprocessTopDom.py $input_path $temp_path/ $chr $resolution $job_id
	#set up driver code
	...
	#Run topdom
	taskset -c 0-100 Rscript "${topdom_path}"
	rm -rf ${output_path}/TopDom.domain
	rm -rf ${output_path}/TopDom.binSignal
	taskset -c 0-100 python3 $home_path/Analysis/cleanTopDom.py $output_path/TopDom.bed $output_path
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/TopDom.bed >> $log_path
	cd $home_path
...
then
	...
	echo "sink()" >> $gmap_path
	taskset -c 0-100 Rscript "${gmap_path}"
	rm -rf GMAP_${job_id}_${norm_name}.r
	taskset -c 0-100 python3 $home_path/Analysis/GMAPClean.py $output_path/GMAP.txt
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/GMAP.txt >> $log_path
...
then
	...
	taskset -c 0-100 octave --no-gui "${path}"
	rm -rf ic-finder_${job_id}_${norm_name}.m
	taskset -c 0-100 python3 $home_path/Analysis/IC-FinderClean.py $output_path/${norm_name}_sigmaZero5_domains.txt $resolution
	mv $output_path/${norm_name}_sigmaZero5_domains.txt $output_path/IC_Finder.txt
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/IC_Finder.txt >> $log_path 
...
then
	...
	echo "sink()" >> $path
	taskset -c 0-100 Rscript "${path}"
	rm -rf HiCseg_${job_id}_${norm_name}.r
	taskset -c 0-100 python3 $home_path/Analysis/HiCsegClean.py $output_path/HiCseg.txt $resolution $input_path
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/HiCseg.txt >> $log_path
...
then
	...
	taskset -c 0-100 python3 tadtool_bed.py $input_path $temp_path/ insulation $chr $resolution
	window_size=$(($resolution*50))
	taskset -c 0-100 tadtool tads $input_path ${temp_path}/insulation.bin $window_size 2 $temp_path/insulation.values -v
	max_val=$(python $home_path/Analysis/tadtool_value.py $temp_path/insulation.values)
	
	cut_off=$(echo ".4*$max_val"|bc)
	taskset -c 0-100 tadtool tads $input_path ${temp_path}/insulation.bin $window_size $cut_off $temp_path/insulation.results
	taskset -c 0-100 python3 $home_path/Analysis/cleanTADTool.py  $temp_path/insulation.results $output_path Insulation_Score_40
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/Insulation_Score_40.bed >> $log_path
	
	cut_off=$(echo ".5*$max_val"|bc)
	taskset -c 0-100 tadtool tads $input_path ${temp_path}/insulation.bin $window_size $cut_off $temp_path/insulation.results
	taskset -c 0-100 python3 $home_path/Analysis/cleanTADTool.py  $temp_path/insulation.results $output_path Insulation_Score_50
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/Insulation_Score_40.bed >> $log_path

	
	cut_off=$(echo ".6*$max_val"|bc)
	taskset -c 0-100 tadtool tads $input_path ${temp_path}/insulation.bin $window_size $cut_off $temp_path/insulation.results
	taskset -c 0-100 python3 $home_path/Analysis/cleanTADTool.py  $temp_path/insulation.results $output_path Insulation_Score_60
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/Insulation_Score_60.bed >> $log_path
...
then
	...
	taskset -c 0-100 python3 tadtool_bed.py $input_path $temp_path di $chr $resolution 
	window_size=$(($resolution*50))
	taskset -c 0-100 tadtool tads $input_path ${temp_path}/di.bin $window_size 2 $temp_path/di.values -a directionality -v
	max_val=$(python $home_path/Analysis/tadtool_value.py $temp_path/di.values)
	
	cut_off=$(echo ".4*$max_val"|bc)
	taskset -c 0-100 tadtool tads $input_path ${temp_path}/di.bin $window_size $cut_off $temp_path/di.results -a directionality
	taskset -c 0-100 python3 $home_path/Analysis/cleanTADTool.py  $temp_path/di.results $output_path DI_40
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/DI_40.bed >> $log_path

	cut_off=$(echo ".5*$max_val"|bc)
	taskset -c 0-100 tadtool tads $input_path ${temp_path}/di.bin $window_size $cut_off $temp_path/di.results -a directionality
	taskset -c 0-100 python3 $home_path/Analysis/cleanTADTool.py  $temp_path/di.results $output_path DI_50
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/DI_50.bed >> $log_path
	
	cut_off=$(echo ".6*$max_val"|bc)
	taskset -c 0-100 tadtool tads $input_path ${temp_path}/di.bin $window_size $cut_off $temp_path/di.results -a directionality
	taskset -c 0-100 python3 $home_path/Analysis/cleanTADTool.py  $temp_path/di.results $output_path DI_60
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/DI_60.bed >> $log_path
...
then
	...
	taskset -c 0-100 hicFindTADs --matrix ${cool_or_h5_path} --outPrefix HICExplorer_${norm_name} --correctForMultipleTesting fdr --chromosomes $chr
	taskset -c 0-100 python3 $home_path/Analysis/HiCExplorerClean.py $additional_file_path/HICExplorer_${norm_name}_domains.bed $resolution $output_path/HiCExplorer.bed
...
then
	cd $Caller_path
	taskset -c 0-100 python3 $home_path/Analysis/shift_nxn.py $input_path $temp_path spectral.matrix $chr $resolution
	...
	taskset -c 0-100 Rscript "${spectral_path}"
	rm -rf spectral_${job_id}_${norm_name}.r
	taskset -c 0-100 python3 $home_path/Analysis/cleanSpectral.py $temp_path/spectral.bed  $output_path
	taskset -c 0-100 python3 $home_path/Analysis/check_bed_error.py $output_path/Spectral.bed >> $log_path
